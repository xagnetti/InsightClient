package net.sakri.flash.bitmap
{
    ...
    public class BitmapDataUtil
    {
        ...
        public static function getNonTransparentPoints( bmd : flash.display.BitmapData, grab_every : uint = 2, resize_percent : Number =
            1, offset : Point = null ) : Vector.<Point>
        {
            if ( offset == null )
         ...
         */
        public static function toMonoChrome( source : flash.display.BitmapData, mono_color : uint = 0xFF000000 ) : flash.
            display.BitmapData
        {
        ...
        public static function toMonoChrome2( source : flash.display.BitmapData, mono_color : uint = 0xFF000000 ) : flash.
            display.BitmapData
        {
            var bmd : flash.display.BitmapData = source.clone();
        ...
        public static function stripTransparentEdgeFromTop( bm : flash.display.BitmapData ) : flash.display.BitmapData
        {
            ...
            if ( first_colored == 0 )
                return bm;
            var stripped : flash.display.BitmapData = new flash.display.BitmapData( bm.width, bm.height - first_colored,
                true, 0x00000000 );
        ...
        public static function stripTransparentEdgeFromLeft( bm : flash.display.BitmapData ) : flash.display.BitmapData
        {
            ...
            if ( first_colored == 0 )
                return bm;
            var stripped : flash.display.BitmapData = new flash.display.BitmapData( bm.width - first_colored, bm.height,
                true, 0x00000000 );
        ...
        public static function getFirstNonTransparentPixelFloodFill( bmd : flash.display.BitmapData, test_fill_color : uint =
            0xFFFFFF00 ) : Point
        {
            //var hit_bmd:BitmapData=new BitmapData(bmd.width,1,true,0);
        ...
        public static function getFirstNonTransparentPixelHitTest( bmd : flash.display.BitmapData, test_fill_color : uint =
            0xFFFFFF00 ) : Point
        {
            var hit_rect : Rectangle = new Rectangle( 0, 0, bmd.width, 1 );
            ...
            for ( iy = 0; iy < bmd_height; iy++ )
            {
                for ( ix = 0; ix < bmd_width; ix++ )
                {
                    if ( bmd.getPixel32( ix, iy ) > 0x00000000 )
                    {
                        ...
                        else if ( !( bmd.getPixel32( ix - 1, iy ) > 0x00000000 && bmd.getPixel32( ix + 1, iy ) > 0x00000000 &&
                            bmd.getPixel32( ix, iy - 1 ) > 0x00000000 && bmd.getPixel32( ix, iy + 1 ) > 0x00000000 ) )
                        {
                            update.push( new Point( ix, iy ) );
                        }
                    }
