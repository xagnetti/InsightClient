package net.sakri.flash.vector
{
    ...
    public class VectorShape
    {
        ...
        {
            if ( a.type != VectorLine.DIAGONAL || b.type != VectorLine.DIAGONAL )
                return false;
        ...
        public function optimizeUsingBreakPoints() : void
        {
            ...
            for ( var i : uint = 1; i < lines_copy.length; i++ )
            {
                ...
                if ( line.start_point.equals( breaks_copy[ bp_index ] ) )
                {
                    ...
                    if ( bp_index >= breaks_copy.length )
                        break;
                }
            }
        ...
        protected function isDiagonal( lines_vect : Vector.<VectorLine>, index1 : uint, index2 : uint ) : Boolean
        {
            ...
            var diff : Number = Point.distance( first, last ) - Point.distance( first, middle ) - Point.distance( middle,
                last );
        ...
        protected function getVectorLinesInCurve( lines : Vector.<VectorLine>, index1 : uint, index2 : uint ) : Vector.<VectorLine>
        {
            ...
            {
                if ( Point.distance( VectorLine( lines[ anchor ] ).start_point, VectorLine( lines[ i ] ).start_point ) >
                    _curve_accuracy )
                {
                    line = new VectorLine( VectorLine( lines[ anchor ] ).start_point, VectorLine( lines[ i ] ).start_point,
                        VectorLine.DIAGONAL );
                    found.push( line );
                    anchor = i
                }
            ...
            if ( line == null )
            {
                line = new VectorLine( VectorLine( lines[ index1 ] ).start_point, VectorLine( lines[ index2 ] ).start_point,
                    VectorLine.DIAGONAL );
                found.push( line );
                return found;
            }
