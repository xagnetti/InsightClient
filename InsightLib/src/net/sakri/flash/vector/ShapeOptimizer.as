package net.sakri.flash.vector
{
    ...
    public class ShapeOptimizer
    {
        ...
        public static function getOptimizedVectorShapeFromPoints( points : Vector.<Point>, shapeFidality : uint = ShapeFidality.
            MEDIUM ) : VectorShape
        {
        ...
        private static function createVectorShapeFromPrunedPoints( points : Vector.<Point> ) : VectorShape
        {
            ...
            for ( var i : uint = 1; i < points.length; i++ )
            {
                ...
                if ( prev.x == cur.x )
                {
                    line = new VectorLine( prev, cur, VectorLine.VERTICAL, ( cur.y > prev.y ? VectorLine.DOWN : VectorLine.
                        UP ) );
                }
                else
                {
                    line = new VectorLine( prev, cur, VectorLine.HORIZONTAL, ( cur.x > prev.x ? VectorLine.RIGHT : VectorLine.
                        LEFT ) );
                }
                lines.push( line );
                prev = cur;
        ...
        protected static function addBreakPoints( points : Object, vs : VectorShape ) : void
        {
            ...
            for ( i = i + 1; i < lines.length; i++ )
            {
                ...
                if ( line.type == VectorLine.HORIZONTAL )
                {
                    if ( line.direction != prev_horizontal.direction )
                        register_break_point = true;
                    prev_horizontal = line;
                }
                else
                {
                    if ( line.direction != prev_vertical.direction )
                        register_break_point = true;
                    prev_vertical = line;
                }
                ...
                if ( register_break_point )
                {
                    ...
                    if ( points[ point_string ] == null )
                        points[ point_string ] = 1;
