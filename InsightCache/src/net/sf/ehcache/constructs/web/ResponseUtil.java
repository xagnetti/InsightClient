...
/**
 * A collection of response processing utilities, which are shared between 2 or
 * more filters
 * 
 * @author Greg Luck
...
public final class ResponseUtil
{
    ...
    * <p/>
    * On a unix system to reproduce do <code>gzip -n empty_file</code>. -n tells
    * gzip to not include the file name. The resulting file size is 20 bytes.
    * <p/>
    * Therefore 20 bytes can be used indicate that the gzip byte[] will be empty
    * when ungzipped.
    */
                                           ...
                                           int responseStatus )
   {
      ...
      if ( responseStatus == HttpServletResponse.SC_NO_CONTENT )
      {
         ...
         {
            LOG.debug( request.getRequestURL()
                  + " resulted in a " + HttpServletResponse.SC_NO_CONTENT
      ...
      if ( responseStatus == HttpServletResponse.SC_NOT_MODIFIED )
      {
         ...
         {
            LOG.debug( request.getRequestURL()
                  + " resulted in a " + HttpServletResponse.SC_NOT_MODIFIED
                  + " response. Removing message body in accordance with RFC2616." );
         }
         return true;
      }
   ...
   /**
    * Adds the gzip HTTP header to the response. This is need when a gzipped
    * body is returned so that browsers can properly decompress it.
    * <p/>
    * 
    * @param response the response which will have a header added to it. I.e
    *           this method changes its parameter
    * @throws ResponseHeadersNotModifiableException Either the response is
    *            committed or we were called using the include method from a
