...
public class PageInfo implements Serializable
{
    ...
    */
   public static boolean isGzipped( final byte[] candidate )
   {
      if ( ( candidate == null )
            || ( candidate.length < 2 ) )
         return false;
    ...
    * @param cookies
    * @param body
    * @param storeGzipped set this to false for images and page fragments which
    *           should never
    * @param timeToLiveSeconds the time to Live in seconds. 0 means maximum,
    *           which is one year per RFC2616.
    * @param headers
    * @throws AlreadyGzippedException
    */
   public PageInfo( final int statusCode,
   ...
   {
      // Note that the ordering is switched with headers at the end to deal with
      // the erasure issues with Java generics causing
    ...
    * @throws AlreadyGzippedException
    * @deprecated use
    *             {@link #PageInfo(int, String, Collection, Collection, Collection, Collection, byte[], boolean, long)}
    */
   @Deprecated
   public PageInfo( final int statusCode,
                    final String contentType,
   ...
   public List getResponseHeaders()
   {
      ...
      for ( final Header< ? extends Serializable > header : this.responseHeaders )
         ...
         case INT:
            headers.add( new String[]
            { header.getName(),
                        ( ( Integer ) header.getValue() ).toString() } );
            break;
         default:
                              ...
                              final long timeToLiveSeconds ) throws AlreadyGzippedException
   {
      ...
      try
      {
         ...
         else if ( isBodyParameterGzipped() )
      ...
      {
         LOG.error( "Error ungzipping gzipped body",
                    e );
      }
   ...
   private byte[] ungzip( final byte[] gzipped ) throws IOException
   {
      ...
      final byte[] ungzipped = byteArrayOutputStream.toByteArray();
      inputStream.close();
